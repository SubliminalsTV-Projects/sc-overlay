...
app.whenReady().then(() => {
  ...
  // Known, accepted limitation: an entirely achromatic (pure white/grey) pin cannot be told apart
  // from an equally achromatic number or background by COLOUR alone — nothing about saturation can
  // recover that without shape detection. Documented here as a boundary, not silently regressed:
  // this scene was never observed in practice (unlike the gold/white one above, which was), and
  // the old hue-matching approach could not have told apart a white pin from a differently-shaded
  // white number either.
  ok("a pure white pin (fully achromatic) is not found — known limitation, not a regression",
