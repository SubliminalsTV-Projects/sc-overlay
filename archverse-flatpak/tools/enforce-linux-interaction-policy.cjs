...
//  6. Flatpak interaction must not depend on raw /dev/input readability. Preserve evdev unchanged
//     whenever the sandbox can really open a keyboard event node; otherwise use the packaged
//     X11/XWayland key-state fallback. Backend selection is capability-based, never distro-based.
//
...
// Durable Linux held-key backend policy. Raw evdev remains the primary path where it already works.
// Flatpak's device exposure cannot override host Unix ACL/group policy, so Fedora/Nobara and some
// Debian-family configurations may still deny open(2). Probe the exact keyboard event nodes before
// starting evdev. Only when no keyboard node can be opened read-only/nonblocking do we start X11.
// This keeps working Arch/CachyOS installs on their existing backend while making the same Flatpak
// self-contained on hosts with stricter /dev/input policy.
...
must(main.includes('linuxReadableEvdevKeyboard()'), 'capability-based evdev probe missing');
must(main.includes('[input-backend] readable evdev keyboard found:'), 'evdev-preservation diagnostic missing');
must(main.includes('[input-backend] no readable evdev keyboard; selecting packaged X11/XWayland held-key fallback'), 'X11 fallback-selection diagnostic missing');
...
must(main.includes('x11InteractController = startLinuxX11HoldKey({ accelerator: accel'), 'X11 held-key backend is not started on evdev failure');

fs.writeFileSync(mainPath, main);
console.log('Linux hover-scoped interaction and capability-selected held-key policies applied and verified');
