...
assert.match(polling, /needsGamePointer[\s\S]*fHoverPointerPhase !== "host"/);
...
assert.match(
  main,
  /fHoverPointerPhase !== "host"[\s\S]*moveHostPointer\?\.\(lastGlobalPointer\)[\s\S]*fHoverPointerPhase = "host"[\s\S]*focusLinuxInteractiveWindow\("overlay"\)/,
);
assert.match(
  main,
